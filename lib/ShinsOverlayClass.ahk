...
ApplyTintManual(image, tintR, tintG, tintB) {
    ; Get the cached bitmap - but we need the original GDI+ bitmap, not the D2D one
    if (!this.imageCache.Has(image)) {
        this.cacheImage(image) ; Ensure image is cached first
    }
    
    ; We need to reload the original image since cacheImage converts it to D2D bitmap
    ; and we need the GDI+ bitmap for pixel manipulation
    w := h := bm := 0
    DllCall("gdiplus\GdipCreateBitmapFromFile", "Str", image, "Ptr*", &bm)
    if (!bm) {
        this.Err("ApplyTintManual: Error", "Could not load image: " image)
        return false
    }
    ...
    r := Buffer(16, 0)
    NumPut("int", 0, r, 0)    ; left
    NumPut("int", 0, r, 4)    ; top  
    NumPut("int", w, r, 8)    ; right
    NumPut("int", h, r, 12)   ; bottom

    ; Lock the bitmap bits
    bmdata := Buffer(32, 0)
    lockResult := DllCall("Gdiplus\GdipBitmapLockBits", "Ptr", bm, "Ptr", r, "uint", 3, "int", 0x26200A, "Ptr", bmdata)
    if (lockResult != 0) {
        this.Err("ApplyTintManual: Error", "Failed to lock bitmap bits. Error code: " lockResult)
        DllCall("gdiplus\GdipDisposeImage", "ptr", bm)
        return false
    }

    scan := NumGet(bmdata, 16, "Ptr")
    stride := NumGet(bmdata, 8, "int")
    pixelFormat := NumGet(bmdata, 12, "int")
    ...
    Loop h {
        ...
        Loop w {
            ...
            ; Read BGRA (GDI+ format)
            b := NumGet(scan + offset, 0, "uchar")
            g := NumGet(scan + offset, 1, "uchar")
            r_val := NumGet(scan + offset, 2, "uchar")
            a := NumGet(scan + offset, 3, "uchar")

            ; Apply tint (multiply each channel and ensure within bounds)
            r_val := Min(255, Round(r_val * tintR))
            g := Min(255, Round(g * tintG))
            b := Min(255, Round(b * tintB))

            ; Write back (BGRA format)
            NumPut("uchar", b, scan + offset, 0)
            NumPut("uchar", g, scan + offset, 1)
            NumPut("uchar", r_val, scan + offset, 2)
            NumPut("uchar", a, scan + offset, 3)
        }
    }

    ; Unlock the bitmap bits
    unlockResult := DllCall("Gdiplus\GdipBitmapUnlockBits", "Ptr", bm, "Ptr", bmdata)
    if (unlockResult != 0) {
        this.Err("ApplyTintManual: Error", "Failed to unlock bitmap bits. Error code: " unlockResult)
        DllCall("gdiplus\GdipDisposeImage", "ptr", bm)
        return false
    }

    ; Clear the cached D2D bitmap
    if (this.imageCache.Has(image)) {
        if (this.imageCache[image]["p"] && this.vTable(this.imageCache[image]["p"], 2)) {
            DllCall(this.vTable(this.imageCache[image]["p"], 2), "ptr", this.imageCache[image]["p"])
        }
        this.imageCache.Delete(image)
    }

    ; Save the modified bitmap to a temporary file and recache it
    ; This is necessary because cacheImage expects a file path
    tempFile := A_Temp "\ShinsOverlay_temp_" A_TickCount ".png"
    
    ; Get image encoder CLSID for PNG
    this._guid("{557CF406-1A04-11D3-9A73-0000F81EF32E}", &clsidPng)
    
    ; Save the modified bitmap
    saveResult := DllCall("gdiplus\GdipSaveImageToFile", "Ptr", bm, "WStr", tempFile, "Ptr", clsidPng, "Ptr", 0)
    if (saveResult != 0) {
        this.Err("ApplyTintManual: Error", "Failed to save modified image. Error code: " saveResult)
        DllCall("gdiplus\GdipDisposeImage", "ptr", bm)
        return false
    }

    ; Dispose of the GDI+ bitmap
    DllCall("gdiplus\GdipDisposeImage", "ptr", bm)

    ; Recache from the temporary file
    this.cacheImage(tempFile)
    
    ; Update the cache to point to the original image name but use temp file data
    if (this.imageCache.Has(tempFile)) {
        this.imageCache[image] := this.imageCache[tempFile].Clone()
        this.imageCache.Delete(tempFile)
    }

    ; Clean up temp file after a short delay
    SetTimer () => FileDelete(tempFile), -1000
