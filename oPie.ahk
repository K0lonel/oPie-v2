...
circle := A_ScriptDir "\assets\circle.png"
glow := A_ScriptDir "\assets\glow.png"
pointer := A_ScriptDir "\assets\pointer.png"
iglow := A_ScriptDir "\assets\iglow.png"
oglow := A_ScriptDir "\assets\oglow.png"
border := A_ScriptDir "\assets\borderlo.png"

diamond := A_ScriptDir "\assets\macro\diamond.png"
color := GetDominantVibrantColor(diamond, alpha := 255)
CreateTintedVariant(border, "borderTint", color.r, color.g, color.b)
SetTimer render, 1
...
render() {
    ...
    if (isKeyDown)
        if (oPie_draw.BeginDraw()) {
            ...
            oPie_draw.DrawImage(circle , anchor_curX, anchor_curY, 100 * scale, 100 * scale, , , , , 0.5, 1, rotation)
            oPie_draw.DrawImage(glow   , anchor_curX, anchor_curY, 200 * scale, 200 * scale, , , , , 0.5, 1, rotation)
            oPie_draw.DrawImage(pointer, anchor_curX + 10 * scale * Cos(mouseAngle * (pi / 180)), anchor_curY + 10 * scale * Sin(mouseAngle * (pi / 180)), 250 * scale, 250 * scale, , , , , 0.5, 1, mouseAngle+90)
            ...
            loop itemCount {
                ...
                targetX := anchor_curX + Cos(angleRad) * radX
                targetY := anchor_curY + Sin(angleRad) * radY

                drawSize := squareSize, color := 0xFFAA0000
                if (selected)  {
                  drawSize := squareSize * 1.5, color := 0xFF00FF00
                  oPie_draw.DrawImage(iglow, Floor(targetX - (drawSize / 2)), Floor(targetY - (drawSize / 2)), drawSize, drawSize,,,,, 0.3)
                  oPie_draw.DrawImage(oglow, Floor(targetX - (drawSize / 2)), Floor(targetY - (drawSize / 2)), drawSize, drawSize,,,,, 0.3)
                  
                }
                oPie_draw.DrawImage("borderTint", Floor(targetX - (drawSize / 2)), Floor(targetY - (drawSize / 2)), drawSize, drawSize)
              }
...
;     ApplyTintManual(A_ScriptDir "\assets\circle.png", color.r, color.g, color.b)
;     ApplyTintManual(A_ScriptDir "\assets\glow.png",   color.r, color.g, color.b)
;     ApplyTintManual(A_ScriptDir "\assets\pointer.png", color.r, color.g, color.b)

;     ApplyTintManual(A_ScriptDir "\assets\borderlo.png", color.r, color.g, color.b)
;     ApplyTintManual(A_ScriptDir "\assets\iglow.png",    color.r, color.g, color.b)
;     ApplyTintManual(A_ScriptDir "\assets\oglow.png",    color.r, color.g, color.b)
; }
...
GetDominantVibrantColor(imagePath, alpha := 255) {
    ; 1. Load the image
    pBitmap := 0
    ...
    return {
        hex: hexColor,   ; Use this for FillRectangle
        r: resRGB.r,     ; Use this for CreateTintedVariant
        g: resRGB.g,
        b: resRGB.b
    }
}
...
; ---------------------------------------------------------
; UPDATED FUNCTION: CreateTintedVariant
; ---------------------------------------------------------
CreateTintedVariant(sourcePath, customKey, tintR, tintG, tintB) {
    ...
    ; 2. Create Canvas
    newBm := 0
    DllCall("gdiplus\GdipCreateBitmapFromScan0", "int", w, "int", h, "int", 0, "int", 0x26200A, "Ptr", 0, "Ptr*", &newBm)
    g := 0
    ...
    DllCall("gdiplus\GdipSetImageAttributesColorMatrix", "Ptr", ia, "int", 1, "int", 1, "Ptr", matrix, "Ptr", 0, "int", 0)
    DllCall("gdiplus\GdipDrawImageRectRect", "Ptr", g, "Ptr", bm, "float", 0, "float", 0, "float", w, "float", h, "float", 0, "float", 0, "float", w, "float", h, "int", 2, "Ptr", ia, "Ptr", 0, "Ptr", 0)
    ...
    if (oPie_draw.imageCache.Has(customKey)) {
        if (oPie_draw.imageCache[customKey]["p"] && oPie_draw.vTable(oPie_draw.imageCache[customKey]["p"], 2)) {
            DllCall(oPie_draw.vTable(oPie_draw.imageCache[customKey]["p"], 2), "ptr", oPie_draw.imageCache[customKey]["p"])
        }
    ...
    if (oPie_draw.bits) { 
        ...
        DllCall(oPie_draw.vTable(oPie_draw.renderTarget, 4), "ptr", oPie_draw.renderTarget, "int64", NumGet(bfSize, 0, "int64"), "ptr", pData, "uint", 4 * w, "ptr", d2dProps, "Ptr*", &d2dBitmap)
    } else { 
        DllCall(oPie_draw.vTable(oPie_draw.renderTarget, 4), "ptr", oPie_draw.renderTarget, "uint", w, "uint", h, "ptr", pData, "uint", 4 * w, "ptr", d2dProps, "Ptr*", &d2dBitmap)
    }
